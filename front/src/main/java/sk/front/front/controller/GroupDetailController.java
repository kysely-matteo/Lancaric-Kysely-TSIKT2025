...
public class GroupDetailController {
    ...
    private void loadGroupData() {
        if (currentGroup != null) {
            ...
            memberCountLabel.setText("\uD83D\uDC65: " + currentGroup.getMemberCount());
    ...
    private void handleRemoveMember() {
        MembershipResponse selectedMember = membersListView.getSelectionModel().getSelectedItem();
        if (selectedMember == null) {
            showAlert(Alert.AlertType.WARNING, "Upozornenie", "Vyberte člena pre odstránenie");
            return;
        }

        // Kontrola - nemôžeš odstrániť sám seba
        if (selectedMember.getUserId().equals(currentUser.getUserId())) {
            showAlert(Alert.AlertType.WARNING, "Upozornenie", "Nemôžete odstrániť samého seba");
            return;
        }

        // Dialóg na potvrdenie
        Alert confirmDialog = new Alert(Alert.AlertType.CONFIRMATION);
        confirmDialog.setTitle("Odstrániť člena");
        confirmDialog.setHeaderText("Odstrániť člena: " + selectedMember.getUserName());
        confirmDialog.setContentText("Naozaj chcete odstrániť tohto člena zo skupiny?\nTáto akcia je nevratná.");

        // Pridanie vlastných tlačidiel
        ButtonType buttonTypeYes = new ButtonType("Áno", ButtonBar.ButtonData.OK_DONE);
        ...
        try {
            apiService.removeMemberFromGroup(currentGroup.getGroupId(), selectedMember.getUserId());

            showAlert(Alert.AlertType.INFORMATION, "Úspech",
                    "Člen " + selectedMember.getUserName() + " bol odstránený zo skupiny");

            loadGroupMembers(); // Refresh zoznamu
            loadGroupData(); // Aktualizovať počet členov

        } catch (Exception e) {
            showAlert(Alert.AlertType.ERROR, "Chyba",
                    "Nepodarilo sa odstrániť člena: " + e.getMessage());
        }
    }
    @FXML
    ...
    private void handleViewAnalytics() {
        try {
            ...
            // Použi rovnaký prístup ako v handleBackToDashboard
            // Použi groupNameLabel alebo iný existujúci Node
            Stage stage = (Stage) groupNameLabel.getScene().getWindow();
